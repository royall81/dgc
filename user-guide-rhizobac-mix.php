<div class="container container-md mt-5 mb-5">
    <div class="col-12 text-center pb-5" style="font-size:27px;">
        <h1>User guide <b style="font-weight:800;">Rhizobac Mix</b></h1>
    </div>
</div>
<div class="container textblock basic-header-width  mt-1    py-3 mb-0"  id="_step_1">
    <div class="row p-0 m-0 px-sm-3 mx-sm-3">
        <div class="col-md-11 text-left p-0 py-sm-3  m-2  text-dark mb-0">
            <div class="d-flex bd-highlight">
                <div class="row">
                    <div class="col-md-4 first-column p-1 pl-md-3">
                        <img class="img-fluid basic-header-thumbnail lazyloaded" src="/img/rhizobac-mix.jpg">
                    </div>
                    <div class="col-md-8 p-md-3 second-column">
                        <p>In order to obtain maximum results with our Rhizobac Mix, simply follow the easy steps below.</p>
                        <p>For all product information, see our <a href="/rhizobac-mix">Rhizobacteria</a></p>
                        <p>Use water with a temperature around 24 °C / 75 °F.</p>
                        <p><strong>Conversion</strong></p>
                        <p>1 gram = 0.035 ounces<br>1 liter = 0.26 gallon</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-11 text-left p-0 py-sm-3  m-2  text-dark mb-0">
            <div class="d-flex bd-highlight">
                <div class="row">
                    <div class="col-md-12 p-0 p-md-3 second-column">
                        <h2>Dosage</h2><br>
                        <p><strong>For seedlings and cuttings</strong></p>
                        <p>1 gram per 5 liter, once a week.</p>
                        <p><br><strong>Use of Rhizobac in the growth and flowering period</strong></p>
                        <p>1 gram per 10 liter water, once a week.</p>
                        <p><br><strong>Use as a "medicine"</strong></p>
                        <p>This product is also great to use as a "medicine" when your plants are looking a little less thriving or when they are subjected to stressful situations. Providing them with the bacteria will give the plants an extra boost and they will soon flourish like never before!</p>
                        <p>1 gram per 5 liter, once.</p>
                        <h2 class="mt-4">Additional information</h2><br>
                        <p>Harmless to humans, animals and the environment.</p>
                        <p><strong>Not for internal use • Keep away from children • Wash hands after usage • Store dry and cool</strong>
                        </p>
                        <p>Should you have any questions, please do not hesitate to <a href="/contact">contact us!</a></p>


                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
